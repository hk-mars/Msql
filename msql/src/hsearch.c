...
/* Before using the hash table we must allocate memory for it.
   ...
   becomes zero.  */
int
hcreate_r (long nel, struct hsearch_data *htab)
{
...
hsearch_r 
(
  ENTRY item, ACTION action, ENTRY **retval, struct hsearch_data *htab
)
{
  unsigned int hval;
  unsigned int count;
...
int 
hcreate(struct hsearch_data *htab, long nel)
{
  int ret;
...
ENTRY *
hsearch(struct hsearch_data *htab, ENTRY item, ACTION action)
{
  ENTRY *result;
...
void 
hdestroy (struct hsearch_data *htab)
{
  if (!htab) return;
